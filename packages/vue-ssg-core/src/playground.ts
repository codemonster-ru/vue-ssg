...
function normalizeLegacyVueforgeImports(source: string): string {
  return source
    .replace(/(['"])@codemonster-ru\/vueforge-codeblock\1/g, '$1@codemonster-ru/vueforge-codeblock/view$1')
    .replace(/(['"])@codemonster-ru\/vueforge-playground\1/g, '$1@codemonster-ru/vueforge-playground/ui$1')
}
  ...
  for (const filePath of walkMarkdownFiles(contentRoot)) {
    ...
    while ((match = playgroundBlockRe.exec(markdown)) !== null) {
      ...
      demos.set(id, {
        ...
        files,
        id,
        source: normalizeLegacyVueforgeImports(source)
      })
    }
  }
...
export function writeMarkdownComponentPlaygroundRegistry(
  ...
  lines.push("import { markRaw, type Component } from 'vue'")
  lines.push('')
  ...
  for (const demo of demos) {
    lines.push(`  '${demo.id}': markRaw(${toMarkdownImportVarName(demo.id)}),`)
  }
  lines.push('}')
  lines.push('')
